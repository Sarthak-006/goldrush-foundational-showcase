...
import {
  ...
  covalentFetch,
  filterPortfolioByBalanceRoster,
  isValidEvmAddress,
...
export async function GET(req: NextRequest) {
  ...
  const portfolioItems = filterPortfolioByBalanceRoster(
    (portfolio.data?.items ?? []) as Parameters<
      typeof filterPortfolioByBalanceRoster
    >[0],
    balanceItems
  );
  ...
  const seriesHasValue = series.some((p) => p.value > 0);
  if ((!series.length || !seriesHasValue) && spotUsdTotal > 0) {
