...
export function StatsRibbon({
  ...
  const lastKick =
    row?.latest_transaction?.block_signed_at != null
      ? row.latest_transaction.block_signed_at.slice(0, 10)
      : "—";
          ...
          if (isApprovals && canOpenApprovals) {
            return (
              <motion.div
                ...
                className="cursor-pointer rounded-2xl border border-amber-500/30 bg-amber-950/20 p-4 text-left backdrop-blur-sm transition hover:border-amber-400/50 hover:bg-amber-950/35 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/60"
              >
                {inner}
              </motion.div>
            );
